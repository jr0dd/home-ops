...
zone_id=$(curl -s -X GET \
    "https://api.cloudflare.com/client/v4/zones?name=${CF_RECORD_NAME#*.}&status=active" \
...
update_ipv4=$(curl -s -X PUT \
    ...
    -H "Authorization: Bearer $CF_DNS_API_TOKEN" \
    -H "Content-Type: application/json" \
    --data "{\"id\":\"${zone_id}\",\"type\":\"A\",\"proxied\":true,\"name\":\"${CF_RECORD_NAME}\",\"content\":\"${current_ipv4}\"}" \
)
